//本章节个人数据过于繁杂，庞大，同学们在开发期间，拷贝代码即可，我们只写关键部位的代码
<template>
  <div class="user-info">
    <!-- 个人信息 -->
...
export default {
  ...
  methods: {
    async getUserDetailById() {
      ...
      //空格的话转化boolean类型是true，如果是原来的一个判断那么就能进去了，这就出问题了
      //所以添加一个trim(),空格trim之后就是空字符串，那么就是false
    ...
    async saveUser() {
      ...
      if (fileList && fileList.length) {
        this.userInfo.staffPhoto = fileList[0].url; //修改成新图片，或者没有修改
      } else {
        this.userInfo.staffPhoto = " "; //删除了图片
      }
  ...
  created() {
    this.getPersonalDetail(); //获取个人信息的详情信息
    this.getUserDetailById(); //获取个人信息的基本信息
  },
};
</script>
