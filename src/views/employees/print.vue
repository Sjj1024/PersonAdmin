...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      ...
      const personInfo = await getPersonalDetail(this.userId); // 获取个人基本信息
      this.formData = { ...personInfo, ...userInfo }; //这个顺序不能写错，personInfo里的username等数据都是null，写后边会给userInfo的username覆盖掉
    },
    async getJobDetail() {
      const userInfo = await getUserDetailById(this.userId);
